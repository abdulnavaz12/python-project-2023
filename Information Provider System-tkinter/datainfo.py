...
import pymysql
mydb=pymysql.Connect(host="localhost",user="root",password="Abdul@123",database="mydata1")
cur=mydb.cursor()
...
def about():
    ...
    text.pack(fill=X)
    text1=Label(us,text='''human good health infomation provider service\n
* This app created by abdul navaz(B.E mechanical)\n
*creater conduct number is 7904106588\n
*address:no 15,89th street,kamarajar salai,ashok nagar,chennai-600083''',font='arial 12 bold',bg='#ffffff',border=0)
...
def mainlog():
    ...
    cur.execute(query)
    for i in cur:
        if (i[1]==name1 and i[4]==int(es.get()) and (cmb.get())=='ADMIN'):
            login1()
...
def login1():
    ...
    cur.execute(query)
    for i in cur:
        if (i[1]==(e1.get()) and i[4]==int(E2.get())and us=='ADMIN'):
            print("Login Success!")
            global input_entry
            print('i am admin')
            r=Toplevel(my)
        ...
        elif  (i[1]==(e1.get()) and i[4]==int(E2.get())and us=='USER') or (i[1]==(e10.get()) and i[4]==int(es.get())and (cmb.get())=='USER'):
            print("Login Success!")
...
def fra1():
    ...
    ph=e3.get()
    password=e4.get()
    query="insert into user (username,email,ph,password) values(%s,%s,%s,%s)"
    values=[username,email,ph,password]
    cur.execute(query,values)
    mydb.commit()
    print('register in database')
    o=e4.get()
    b=e5.get()
    if o==b:
        print('hi')
...
def got():
    global e3,e4,e5,a7,E2,a4,e11,e2
    #c1=e3.get()
    if cmb.get()=='ADMIN'or cmb.get()=='USER':
        ...
        e11=Entry(frame1,font=('times',15),bg='white',border=0)
...
global e3,e2,img5,cmb,e1,e2,e10,es,name1,password1

def main():
    global cmb,e10,es,name1,password1
    w=Toplevel(my)
    w.title('first page')
    w.geometry('925x500')
    w.config(bg="#fff")
    ...
    e10=Entry(frame,fg='black',border=0,bg="white",font=("Microsoft YaHei uI Light",11))
    e10.place(x=30,y=90)
    e10.insert(0,'username')
    e10.bind('<FocusIn>',on_enter)
    e10.bind('<FocusOut>',on_leave)

    Frame(frame,width=295,height=2,bg='black').place(x=25,y=115)

    es=Entry(frame,font=("Microsoft YaHei uI Light",11),bg='white',border=0,fg='black',width=25)
    es.place(x=30,y=150)
    es.insert(0,'password')
    es.bind('<FocusIn>',on_enter1)
    es.bind('<FocusOut>',on_leave1)

    Frame(frame,width=295,height=2,bg='black').place(x=25,y=177)
    b1=Button(frame,width=42,text='LOGIN',bg='#57a1f8',fg='white',border=0,command=mainlog)
    b1.place(x=30,y=225)
